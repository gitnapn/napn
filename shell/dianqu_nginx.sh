...
app_list=("ltePortal" "appPortal" "ltePad" "pubPortal" "provPortal" "portalService" "channelService" "RestCrmExtrnlService" "PrvncCrmExtrnlService" "PrvncCrmInnerService" "RuleManagerWeb" "RuleWeb" "smService" "SoWeb" "SRHttpServiceWeb" "casw" "ecs_appmanager" "ecs_appmanager_config" "LTE-CSB")
...
function Usage(){
	...
	echo " 	sh $0 app  [all|tePortal|appPortal|ltePad|pubPortal|provPortal|portalService|channelService|RestCrmExtrnlService|PrvncCrmExtrnlService|PrvncCrmInnerService|RuleManagerWeb|RuleWeb|smService|SoWeb|SRHttpServiceWeb|casw|ecs_appmanager|ecs_appmanager_config|LTE-CSB] [A,a|B,b] # 切换app版本 "
	exit 2
}
...
function service_version(){
	...
	do
		...
		version=`grep  $app  ${route_file}|awk '{print $2}'|awk -F '_' '{print "_"$NF}' |tr [a-z] [A-Z] `
		echo  " ${app}${version}"
	done
